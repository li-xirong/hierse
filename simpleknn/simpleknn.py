...
class search_model(Structure):
    ...
    def search_knn(self, query, max_hits):
        ...
        logger.debug("search %d-nn, %.4f seconds", topn, knn_time)
...
def load_model(feat_dir):
    ...
    if not model:
        logger.error("failed to load a search model from %s", model_file_name)
        return None

    model = toPyModel(model)
    model.load_ids(os.path.join(feat_dir, 'id.txt'))
    model.set_distance('l2')
    logger.info('%dx%d', nr_of_images, feat_dim)
    return model
